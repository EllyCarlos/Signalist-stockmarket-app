...
type SearchCandidate = FinnhubSearchResult & {
    exchange?: string;
};
...
async function fetchJSON<T>(url: string, revalidateSeconds?: number): Promise<T> {
    ...
    const timeoutId = setTimeout(() => controller.abort(), FINNHUB_REQUEST_TIMEOUT_MS);
    ...
    try {
        ...
        if (error instanceof Error && error.name === 'AbortError') {
            throw new Error(`Finnhub request timed out after ${FINNHUB_REQUEST_TIMEOUT_MS}ms`);
        }
        throw error;
    } finally {
    ...
    try {
        ...
        if (!trimmed) {
            ...
            results = profiles
                ...
                .filter((x): x is SearchCandidate => Boolean(x));
        } else {
